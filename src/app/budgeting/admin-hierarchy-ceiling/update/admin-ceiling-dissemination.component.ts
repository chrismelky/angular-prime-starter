...
export class AdminCeilingDisseminationComponent implements OnInit {
  ...
  constructor(
    ...
    this.ceilingStartSectionPosition =
      this.config.data.ceilingStartSectionPosition;
  ...
  loadCeilingChain() {
    this.ceilingChainService
      ...
      .subscribe((resp: CustomResponse<CeilingChain[]>) => {
        ...
        this.ceilingChain = (resp.data ?? []).filter(
          (cc) =>
            cc.section_level_position.position >
            this.ceiling!.section?.position!
        );
        this.currentCeilingChain = (resp.data ?? []).find(
          (cc) =>
            cc.section_level_position.position ==
            this.ceiling!.section?.position!
        );
        this.selectedCeiling[
          this.currentCeilingChain!.section_level_position.position
        ] = { ...this.ceiling };
  ...
  onchange(ceiling: any, chain: CeilingChain) {
    this.selectedCeiling[chain.section_level_position.position] = {
      ...ceiling,
    };
    if (chain.next_id) {
  ...
  getChildCeiling(ceiling: AdminHierarchyCeiling, chain: CeilingChain = {}) {
    this.adminHierarchyCeilingService
      ...
      .subscribe((res: CustomResponse<AdminHierarchyCeiling[]>) => {
        this.toAllocate = res.data ?? [];
        this.toAllocate = this.toAllocate!.map((c) =>
          Object.assign(c, {
            percent:
              ceiling.amount! > 0 ? (c.amount! / ceiling!.amount!) * 100 : 0,
          })
        );
        this.totalAllocatedCeiling[chain.section_level_position.position] = {
          ...{ amount: this.getTotalAllocatedAmount(this.toAllocate!) },
        };
        this.adminCeilingByLevel[chain.next.section_level_position.position] =
          this.toAllocate;
        this.clonedCeiling = this.toAllocate!.map((c) => ({
          id: c.id,
          amount: c.amount,
          percent: c.percent,
          section: c.section,
  ...
  updateFacilityCeiling(row: any): void {
    ...
    if (this.clonedFacilityCeiling![index].amount != row.amount) {
      if (this.totalFacilityAllocatedAmount! <= this.finalCeiling!.amount!) {
        ...
        this.facilityCeiling![index].amount =
          this.clonedFacilityCeiling![index].amount;
        this.facilityCeiling![index].percent =
          this.clonedFacilityCeiling![index].percent;
        this.getFacilityPercent(
          this.clonedFacilityCeiling![index],
          this.clonedFacilityCeiling![index].percent!
        );
      }
    }
  ...
  updateCeiling(adminHierarchyCeiling: AdminHierarchyCeiling) {
    const index = this.toAllocate!.findIndex(
      (item) => item.id === adminHierarchyCeiling.id
    );
    if (this.clonedCeiling![index].amount != adminHierarchyCeiling.amount) {
      if (
        this.totalAllocatedCeiling[this.allocationPosition!].amount <=
        this.selectedCeiling[this.allocationPosition!]!.amount!
      ) {
        const ceiling = this.updateFromForm(adminHierarchyCeiling);
        this.subscribeToSaveResponse(
          this.adminHierarchyCeilingService.update(ceiling),
        ...
        this.getPercent(
          this.clonedCeiling![index],
          this.clonedCeiling![index].percent!
        );
      }
    }
  }
  ...
  facilityCeilingChange(row: any, amount: number): void {
    ...
    this.facilityCeiling![i].percent =
      this.finalCeiling!.amount! > 0
        ? (amount! / this.finalCeiling!.amount!) * 100
        : 0.0;
    this.totalFacilityAllocatedAmount = this.getTotalAllocatedAmount(
      this.facilityCeiling!
    );
    this.totalAllocatedCeiling[this.finalCeiling?.section?.position] = {
      ...{ amount: this.getTotalAllocatedAmount(this.facilityCeiling!) },
    };
  }

  //Ceiling Input Change
  ceilingChange(row: AdminHierarchyCeiling, amount: number): void {
    const position = this.allCeilingChain!.find(
      (cc) => cc.next.section_level_position.position == row.section?.position
    )!.section_level_position.position;
    const i = this.toAllocate!.findIndex((item) => item.id === row.id);
    this.toAllocate![i].amount = amount;
    this.toAllocate![i].percent =
      this.selectedCeiling[position]!.amount! > 0
        ? (amount! / this.selectedCeiling[position].amount!) * 100
        : 0.0;
    this.totalAllocatedCeiling[position] = {
      ...{ amount: this.getTotalAllocatedAmount(this.toAllocate!) },
    };
  }
  ...
  getFacilityPercent(row: any, percent: number): void {
    ...
    this.facilityCeiling![i].amount =
      (percent * this.finalCeiling!.amount!) / 100;
    this.totalFacilityAllocatedAmount = this.getTotalAllocatedAmount(
      this.facilityCeiling!
    );
    this.totalAllocatedCeiling[this.finalCeiling?.section?.position] = {
  ...
  getPercent(row: AdminHierarchyCeiling, percent: number) {
    const position = this.allCeilingChain!.find(
      (c) =>
        c.next_id &&
        c.next.section_level_position.position == row.section?.position
    )?.section_level_position.position;
    const i = this.toAllocate!.findIndex((item) => item.id === row.id);
    this.toAllocate![i].percent = percent;
    this.toAllocate![i].amount =
      (percent * this.selectedCeiling[position!].amount!) / 100;
    this.adminCeilingByLevel[row.section?.position!][i].amount =
      (percent * this.selectedCeiling[position!].amount!) / 100;
    this.totalAllocatedCeiling[position] = {
      ...{ amount: this.getTotalAllocatedAmount(this.toAllocate!) },
    };
  ...
  getTotalAllocatedAmount(data: any[]) {
    return data.reduce(
      (total, ceiling) => Number(total) + Number(ceiling!.amount),
      0
    );
  }
  ...
  public updateFromForm(ceiling: any): AdminHierarchyCeiling {
    let ceilingChain = this.ceilingChain!.filter(
      (chain) =>
        chain.section_level_position.position === ceiling!.section!.position!
    )[0];
    return {
  ...
  protected onSaveSuccess(result: any, ceiling: any): void {
    if ('facility' in ceiling) {
      const index = this.facilityCeiling!.findIndex(
        (item) => item.id === ceiling.id
      );
      this.clonedFacilityCeiling = this.facilityCeiling!.map((c) => ({
        id: c.id,
        amount: c.amount,
        percent: c.percent,
      }));
          ...
          this.budgetCeilingService
            ...
            .subscribe((res: any) => {
              ...
              this.facilityCeiling = this.facilities!.map((facility) => {
                const budgetCeiling = this.budgetCeiling!.find(
                  (ceiling) => ceiling.facility_id === facility.id
                );
                const position = 'p' + facility.planning_hierarchy_position;
                return {
                  id: facility.id,
                  ceilingId:
                    budgetCeiling == undefined ? null : budgetCeiling!.id,
                  ...
                  percent:
                    budgetCeiling == undefined
                      ? 0.0
                      : budgetCeiling.amount! > 0
                      ? (budgetCeiling.amount! / ceiling.amount) * 100
                      : 0.0,
                };
              });
              this.clonedFacilityCeiling = this.facilityCeiling!.map((c) => ({
              ...
              this.totalAllocatedCeiling[ceiling?.section?.position] = {
                ...{
                  amount: this.getTotalAllocatedAmount(this.facilityCeiling!),
                },
              };
    ...
    if (
      this.facilityCeilingToUpload
        ...
        .reduce(function (a: any, b: any) {
          return a + b;
        }) <= this.finalCeiling.amount
    ) {
      this.adminHierarchyCeilingService
        .uploadFinalCeiling(ceilings)
      ...
      this.toastService.error(
        'Total Ceiling Amount Uploaded Should be Less Or Equal To Total Ceiling'
      );
    }
  }
  ...
  downloadFacilityCeiTemplate() {
    ...
    if (navigator.msSaveBlob) {
      csvURL = navigator.msSaveBlob(
        csvData,
        this.finalCeiling.section.name + '_facility_ceiling.xlsx'
      );
    } else {
  ...
  downloadHsbfAllocation() {
    ...
    this.adminHierarchyCeilingService
      .downloadHsbfAllocation(payload)
  ...
  confirmHsbfAllocation(event: any) {
    let msg =
      ...
      this.ceiling?.ceiling?.fund_source!.name +
      ' allocation?';
    this.confirmationService.confirm({
      ...
      reject: () => {
        this.toastService.info(
          this.ceiling?.ceiling?.fund_source!.name + ' Allocation Canceled'
        );
      },
    });
  }
