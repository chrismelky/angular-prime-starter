...
export class CasPlanContentComponent implements OnInit {
  ...
  /**
   * Creating or updating CasPlanContent
   * @param casPlanContent ; If undefined initize new model to create else edit existing model
   */
  createOrUpdate(casPlanContent?: CasPlanContent): void {
    const data: CasPlanContent = casPlanContent ?? {
      ...new CasPlanContent(),
      cas_plan_id: this.cas_plan_id,
    };
