...
@Component({
  ...
  templateUrl: "./pe-select-option.component.html",
})
export class PeSelectOptionComponent implements OnInit {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    this.peSelectOptionService
      ...
      .subscribe(
        (res: CustomResponse<PeSelectOption[]>) => {
  ...
  /**
   * Creating or updating PeSelectOption
   * @param peSelectOption ; If undefined initize new model to create else edit existing model
   */
  createOrUpdate(peSelectOption?: PeSelectOption): void {
    const data: PeSelectOption = peSelectOption ?? {
      ...new PeSelectOption(),
    };
    const ref = this.dialogService.open(PeSelectOptionUpdateComponent, {
      data,
      header: "Create/Update PeSelectOption",
    });
  ...
  /**
   * Delete PeSelectOption
   * @param peSelectOption
   */
  delete(peSelectOption: PeSelectOption): void {
    this.confirmationService.confirm({
      message: "Are you sure that you want to delete this PeSelectOption?",
      accept: () => {
        this.peSelectOptionService
          .delete(peSelectOption.id!)
          .subscribe((resp) => {
            this.loadPage(this.page);
   ...
   */
  protected onSuccess(
    resp: CustomResponse<PeSelectOption[]> | null,
    page: number,
    navigate: boolean
  ): void {
    ...
    this.page = page;
    if (navigate) {
      this.router.navigate(["/pe-select-option"], {
        queryParams: {
          page: this.page,
          per_page: this.per_page,
          sort:
