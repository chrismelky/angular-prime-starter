...
@Component({
  selector: "app-planning-matrix",
  templateUrl: "./planning-matrix.component.html",
})
export class PlanningMatrixComponent implements OnInit {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    this.planningMatrixService
      ...
      .subscribe(
        (res: CustomResponse<PlanningMatrix[]>) => {
  ...
  /**
   * Creating or updating PlanningMatrix
   * @param planningMatrix ; If undefined initize new model to create else edit existing model
   */
  createOrUpdate(planningMatrix?: PlanningMatrix): void {
    const data: PlanningMatrix = planningMatrix ?? {
      ...new PlanningMatrix(),
    };
    const ref = this.dialogService.open(PlanningMatrixUpdateComponent, {
      data,
      header: "Create/Update PlanningMatrix",
    });
   ...
   */
  delete(planningMatrix: PlanningMatrix): void {
    this.confirmationService.confirm({
      message: "Are you sure that you want to delete this PlanningMatrix?",
      accept: () => {
        this.planningMatrixService
          .delete(planningMatrix.id!)
          .subscribe((resp) => {
            this.loadPage(this.page);
   ...
   */
  protected onSuccess(
    resp: CustomResponse<PlanningMatrix[]> | null,
    page: number,
    navigate: boolean
  ): void {
    ...
    this.page = page;
    if (navigate) {
      this.router.navigate(["/planning-matrix"], {
        queryParams: {
          page: this.page,
          per_page: this.per_page,
          sort:
  ...
  protected onError(): void {
    ...
    this.page = 1;
    this.toastService.error("Error loading Planning Matrix");
  }
}
